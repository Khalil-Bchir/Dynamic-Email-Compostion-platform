    ...
    <div class="flex w-full items-center justify-center">
      <form class="max-w-xs" @submit.prevent="onSubmit">
        <div class="w-full max-w-xs self-center pb-11 text-center">
          ...
          <label class="text-destructive bg-transparent text-xs"
            :class="[!authStore.isLoading && authStore.error ? 'block' : 'hidden']">{{ authStore.error?.code ?
        t(authStore.error?.code) : authStore.error?.message }}
          </label>
        </div>
        ...
        t('did_you_forget_password')
          ...
          <Button variant="outline" class="" @click="$router.push('/register')">
            <span>{{ t('sign_up') }}</span>
          </Button>
        </div>
      </form>
